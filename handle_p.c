							...
							int len_of_num, int len_of_f)
{
	...
	if (all.width > 0 && all.prec == -1 && arg == 0)
		white_s--;
	if (all.min == 0)
	{
		if (all.null == 1)
...
t_var				handle_p(t_var all, va_list list)
{
	...
	len_of_f += check_biggest(all.width, all.prec + 2, len_of_num + 2);
	all.var += len_of_f;
	all.prec == -1 && arg == 0 && all.width == 0 ? all.var++ : 0;
